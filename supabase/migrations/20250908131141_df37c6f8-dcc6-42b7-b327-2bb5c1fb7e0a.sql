...
-- Generate codes for existing packages using DO block
DO $$
DECLARE
  rec RECORD;
  counter INTEGER := 1;
BEGIN
  FOR rec IN SELECT id FROM public.service_packages ORDER BY created_at LOOP
    UPDATE public.service_packages 
    SET package_code = 'GA' || LPAD(counter::text, 3, '0')
    WHERE id = rec.id;
    counter := counter + 1;
  END LOOP;
END $$;
