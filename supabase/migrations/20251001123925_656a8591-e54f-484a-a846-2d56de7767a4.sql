-- Update execute_checkout_transaction function to set service_package_id when creating farm_rental
CREATE OR REPLACE FUNCTION public.execute_checkout_transaction(p_user_id uuid, p_farm_id uuid, p_package_id text, p_package_name text, p_package_price numeric, p_selected_chicken_type_id uuid, p_selected_chicken_type_name text, p_selected_chicken_quantity integer, p_total_amount numeric, p_coop_id text DEFAULT NULL::text, p_selected_chickens jsonb DEFAULT '{}'::jsonb)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- Create service package record first to get the ID
  INSERT INTO public.service_packages (
    ...
    p_total_amount
  ) RETURNING id INTO v_service_package_id;
  ...
  IF (p_package_id = 'advanced' OR p_package_id = 'vip') AND p_coop_id IS NOT NULL THEN
    ...
    IF FOUND THEN
      ...
      -- Create farm rental record with service_package_id link
  ...
  -- Update the service package with correct coop info
  UPDATE public.service_packages 
  SET coop_name = v_coop_name,
      coop_price = v_coop_price
  WHERE id = v_service_package_id;
