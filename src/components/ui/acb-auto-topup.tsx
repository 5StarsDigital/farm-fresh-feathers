...
export default function ACBAutoTopup() {
  ...
  const checkACB = async () => {
    ...
    try {
      const {
        data,
        error
      } = await supabase.functions.invoke('acb-auto-topup');
      if (error) {
  ...
  return <Card className="border-2 border-primary/20 bg-gradient-to-br from-green-50 to-blue-50">
      <CardHeader>
      ...
      <CardContent className="space-y-4">
          ...
          <div className="space-y-1">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-gray-500">
                {lastCheck ? `Lần cuối: ${lastCheck.toLocaleTimeString()}` : 'Chưa kiểm tra'}
              </span>
            </div>
          </div>
        ...
        {isChecking && <div className="flex items-center justify-center gap-2 text-blue-600 bg-blue-50 p-3 rounded-lg">
